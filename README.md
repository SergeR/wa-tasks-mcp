...
Локальный MCP-сервер для Webasyst Tasks. Рассчитан на использование **одним разработчиком** на локальной машине — сервер работает с одним фиксированным токеном Webasyst Tasks, поддержка нескольких пользователей или ключей API не предусмотрена.

Реализованы базовые операции: просмотр, создание и редактирование задач, комментарии и смена статуса.

## Что умеет

- `list_tasks` — получить список задач (inbox, по проекту, по статусу, поиск)
- `create_task` — создать задачу в проекте
- `update_task` — изменить свойства существующей задачи (название, описание, проект, срок, исполнителя, приоритет, статус)
- `task_action` — закрыть / перевести по workflow / вернуть задачу
...
- `update_comment` — отредактировать комментарий
- `list_comments` — прочитать комментарии к задаче
